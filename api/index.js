...
async function askOnce(openai, { imageBase64, model }) {
  ...
  return { letter, meta: { model: resp?.model || model, id: resp?.id || null, usage: resp?.usage || null } };
}

function majority(votes) {
  const m = new Map();
  for (const v of votes) m.set(v, (m.get(v) || 0) + 1);
  // 返回出现次数最多的字母（若都不同，返回第一个非空或"A"）
  let best = "", bestN = 0;
...
app.post("/answer", async (req, res) => {
  try {
    ...
    // 三种模式：fast / accurate / accurate_vote3
    const isVote3 = mode === "accurate_vote3";
    const model = isVote3 ? MODEL_MAP.accurate : (MODEL_MAP[mode] || MODEL_MAP.fast);
    ...
    const meta = { votes, model, ids: [r1.meta.id, r2.meta.id, r3.meta.id], usage: [r1.meta.usage, r2.meta.usage, r3.meta.usage] };
